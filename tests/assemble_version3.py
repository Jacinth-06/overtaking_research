...
with open('tests/version1.py', 'r') as f:
    lines = f.readlines()

# Find where control_loop starts
start_idx = -1
for i, line in enumerate(lines):
    if line.startswith('def control_loop(car: JetRacer):'):
        start_idx = i
        break

if start_idx == -1:
    print("Could not find control_loop")
    exit(1)

control_loop_and_beyond = lines[start_idx:]

code_str = "".join(control_loop_and_beyond)

# Replace the state machine logic
old_logic = """        # 3. STATE MACHINE (Strictly IF/ELIF, NO WHILE LOOPS)
        if s_copy["enabled"]:
...
code_str = code_str.replace(old_logic, new_logic)

# We also need to add `yaw`, `pos_y`, `last_time`, `pid_traj` initializations at the top of control_loop
old_init = """    cap = open_camera()
    fps_counter, fps_time = 0, time.time()
    frame_idx = 0
    print("[loop] Control loop started")"""

new_init = """    cap = open_camera()
    fps_counter, fps_time = 0, time.time()
    frame_idx = 0
...
code_str = code_str.replace(old_init, new_init)

with open('tests/version3.py', 'a') as f:
    f.write(code_str)

print("Successfully generated and appended to tests/version3.py")
