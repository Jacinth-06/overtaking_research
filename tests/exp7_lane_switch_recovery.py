#!/usr/bin/env python3
"""
exp8_lane_switch_recovery.py — Lane switch with recovery
=========================================================
Car follows the lane using PID for 0.3m. Then uses MPC to switch to the
adjacent lane. It follows the new lane for 0.2m, then uses MPC to
switch back (recover) to the original lane and follows it permanently.

No LiDAR. Pure odometry-based switching sequence.

State machine:  FOLLOW → SWITCHING → FOLLOW_NEW → RECOVERING → FOLLOW_ORIGINAL

Run:   python exp8_lane_switch_recovery.py
...
# ── Maneuver constants ──────────────────────────────────────────────────────
SWITCH_TRIGGER_DIST = 0.3      # follow original lane for 0.3m before switch
FOLLOW_NEW_DIST = 0.2          # follow adjacent lane for 0.2m before recovery
...
def control_loop(car: JetRacer):
    ...
    autonomy_state = "FOLLOW"
    state_start_dist = None

    print("[loop] Lane switch with recovery control loop started")
    ...
    while True:
        ...
        if s_copy["enabled"]:
            if autonomy_state == "FOLLOW":
                if state_start_dist is None: state_start_dist = enc_dist
                travel = enc_dist - state_start_dist
                ...
                if travel >= SWITCH_TRIGGER_DIST:
                    ...
                    mpc_last_v = max(enc_speed, 0.01); mpc_last_time = 0.0
                    ...
                    car.steer(steer); car.forward(s_copy["speed"])
                ...
                if travel >= FOLLOW_NEW_DIST:
                    # Switch back (recover)
                    autonomy_state = "RECOVERING"
                    yaw = 0.0; pos_y = 0.0
                    mpc_prev_delta = 0.0; mpc_last_delta = 0.0
                    mpc_last_v = max(enc_speed, 0.01); mpc_last_time = 0.0
                    mpc_throttle_ref = s_copy["speed"]
                    ...
                    pid_state["start_enc_dist"] = enc_dist
                    pid_state["lane_change_dist"] = OVERTAKE_MANEUVER_DIST
                    print(f"[STATE] FOLLOW_NEW → RECOVERING at enc_dist={enc_dist:.3f}", flush=True)
                else:
                    car.steer(steer); car.forward(s_copy["speed"])

            elif autonomy_state == "RECOVERING":
                s = enc_dist - pid_state.get("start_enc_dist", enc_dist)
                D = pid_state.get("lane_change_dist", OVERTAKE_MANEUVER_DIST)
                ...
                if (s >= D and abs(traj_error) < RECOVERY_POS_TOLERANCE) or s >= MANEUVER_MAX_DIST:
                    print(f"[STATE] RECOVERING → FOLLOW_ORIGINAL (enc_dist={enc_dist:.3f})", flush=True)
                    autonomy_state = "FOLLOW_ORIGINAL"
                    yaw = 0.0; pos_y = 0.0
            ...
            elif autonomy_state == "FOLLOW_ORIGINAL":
                car.steer(steer); car.forward(s_copy["speed"])
...
<title>Exp 7 — Lane Switch & Recovery</title>
...
<div class="header">
  <h1>Exp 7: Lane Switch & Recovery</h1>
...
<script>
const sliders = ["speed","kp","ki","kd","canny_lo","canny_hi","binary_thresh"];
sliders.forEach(id => {
...
async function poll() {
  try {
    ...
    if (d.autonomy_state === "SWITCHING" || d.autonomy_state === "RECOVERING") {
    ...
    with state_lock:
        return jsonify({k: state[k] for k in
                        ("fps","error","steer","lane_found","enabled","autonomy_state",
                         "enc_speed","enc_dist","pos_y","yaw_deg","is_testing","test_id") if k in state})
...
def download_csv():
    ...
    return Response(output.getvalue(), mimetype="text/csv", 
                    headers={"Content-Disposition": "attachment;filename=exp7_telemetry.csv"})
