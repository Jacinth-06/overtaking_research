#!/usr/bin/env python3
"""
exp7_lane_switch_recovery.py — Lane follower + MPC overtaking/recovery + Lidar safety + Flask dashboard
...
def control_loop(car: JetRacer):
    ...
    while True:
        ...
        with _imu_cache_lock:
            imu_ax = _imu_cache["ax"]; imu_ay = _imu_cache["ay"]; imu_az = _imu_cache["az"]
            imu_gx = _imu_cache["gx"]; imu_gy = _imu_cache["gy"]; imu_gz = _imu_cache["gz"]
...
<body>
<h1>&#9675; Exp 7 &mdash; Lane Switch + Recovery</h1>
<div class="grid">
  <div class="card">
