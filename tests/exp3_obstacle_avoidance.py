#!/usr/bin/env python3
"""
exp3_obstacle_avoidance.py — Obstacle avoidance via steering
=============================================================
Car drives straight. When the front LiDAR cone detects an obstacle within
stop_distance, the car steers left for a fixed duration, then straightens
and resumes driving straight.

No camera, no vision pipeline.

State machine:  STRAIGHT → TURNING → COOLDOWN → STRAIGHT

Run:   python exp3_obstacle_avoidance.py
...
# ── Config constants ──────────────────────────────────────────────────────────
TURN_DURATION = 0.8       # seconds to hold the turn
COOLDOWN_DURATION = 0.5   # seconds to drive straight after turning before re-checking
TURN_STEER = -0.7         # steer value during avoidance (negative = left)
...
state = {
    ...
    "enabled": False,
    "stop_distance": 400.0,
    "drive_state": "STRAIGHT",    # STRAIGHT, TURNING, COOLDOWN
    # Lidar
    "lidar_closest": 0.0,
    "lidar_blocked": False,
    # Encoder data
...
def control_loop(car: JetRacer):
    print("[loop] Obstacle avoidance control loop started")
        ...
        if s_copy["enabled"]:
            if drive_state == "STRAIGHT":
                if lidar_blocked:
                    ...
                    print(f"[STATE] STRAIGHT → TURNING (obstacle at {lidar_closest:.0f}mm)", flush=True)
                else:
                    # Drive straight
                    car.steer(0.0)
                    car.forward(s_copy["speed"])

            if drive_state == "TURNING":
                elapsed = now - state_start_time
                if elapsed >= TURN_DURATION:
                    # Turn complete — enter cooldown
                    drive_state = "COOLDOWN"
                    state_start_time = now
                    print("[STATE] TURNING → COOLDOWN", flush=True)
                else:
                    # Steer left while maintaining speed
                    car.steer(TURN_STEER)
                    car.forward(s_copy["speed"])

            elif drive_state == "COOLDOWN":
                elapsed = now - state_start_time
                if elapsed >= COOLDOWN_DURATION:
                    drive_state = "STRAIGHT"
                    state_start_time = now
                    print("[STATE] COOLDOWN → STRAIGHT", flush=True)
                else:
                    # Drive straight during cooldown
                    car.steer(0.0)
                    car.forward(s_copy["speed"])
...
<title>Exp 3 — Obstacle Avoidance</title>
<style>
  :root {
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .turning { color: var(--warn) !important; }
  .blocked { color: var(--danger) !important; }
</style>
</head>
<body>
<h1>&#9675; Exp 3 &mdash; Obstacle Avoidance</h1>
...
<div class="card">
  ...
  <div class="status-bar">
    <div class="stat"><span class="stat-val" id="v-state">STRAIGHT</span>
...
async function poll() {
  try {
    ...
    stateEl.textContent = d.drive_state;
    if (d.drive_state === "TURNING") stateEl.classList.add("turning");
    else stateEl.classList.remove("turning");
...
def status():
    with state_lock:
        return jsonify({k: state[k] for k in
                        ("speed", "enabled", "drive_state", "lidar_closest", "lidar_blocked",
